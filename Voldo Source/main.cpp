...
auto DirectXInit(HWND hWnd) -> HRESULT
{
	...
	Color[ImGuiCol_Header] = ImColor(79, 79, 79, 255);
...
static std::string ReadGetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(modules::BaseAddress + 0xe05970 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
	...
	if ((uint32_t)nameLength)
	{
		...
		v6 = read<DWORD_PTR>(modules::BaseAddress + 0xC34F5A0) >> 5;
		result = 0;
		unsigned int v35;
		if (v4)
		{
...
static std::string GetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(modules::BaseAddress + 0xc4821c0 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
...
// all offsets are updated to chapter 3 season 2
		offests::uworld = read<uint64_t>(modules::BaseAddress + 0xc3ef7c8);//updated 
		

		offests::game_instance = read<uint64_t>(offests::uworld + 0x198);//updated 
		

		offests::local_players_array = read<uint64_t>(read<uint64_t>(offests::game_instance + 0x38));//updated 
		

		offests::player_controller = read<uint64_t>(offests::local_players_array + 0x30);//updated 
		

		offests::Pawn = read<uint64_t>(offests::player_controller + 0x348);//updated 
		if (!offests::Pawn)continue;
		

		offests::rootcomponent = read<uint64_t>(offests::Pawn + 0x190);//updated 
		if (!offests::rootcomponent)continue;

		offests::relativelocation = read<Vector3>(offests::rootcomponent + 0x138);//updated 
		if (!IsVec3Valid(offests::relativelocation))continue;

		uintptr_t GameState = read<uintptr_t>(offests::uworld + 0x128);//updated 
		if (!GameState)continue;
		

		uintptr_t PlayerArray = read<uintptr_t>(GameState + 0x243);//updated 
		if (!PlayerArray)continue;
		

		int Count = read<int>(GameState + (0x258 + sizeof(uint64_t)));//updated 
		...
		for (int i = 0; i < Count; i++)
		{
			...
			uintptr_t CurrentPawn = read<uintptr_t>(CurrentPlayerState + 0x320);//updated 
			if (!CurrentPawn) continue;

			uint64_t mesh = read<uint64_t>(CurrentPawn + 0x318);//updated 
			if (!mesh)continue;
			

			uint32_t curactorid = read<uint32_t>(CurrentPawn + 0x18);//updated 
			if (!curactorid)continue;
			

			uint64_t PlayerState = read<uint64_t>(CurrentPawn + 0x2C8);//updated 
			if (!PlayerState)continue;

			uint64_t LocalPlayerState = read<uint64_t>(offests::Pawn + 0x290);//updated 
			if (!LocalPlayerState)continue;

			std::string Names = GetNameFromFName(curactorid);

			if (mesh != 0x00 && PlayerState != 0x00 && LocalPlayerState != 0x00 || Names.find("PlayerPawn") != std::string::npos) {
				FNlEntity fnlEntity{ };
...
void fn_esp() {
	...
	for (unsigned long i = 0; i < entityListCopy.size(); i++)
	{
		...
		Vector3 relativelocation = read<Vector3>(rootcomponent + 0x135);
...
void menu() {
	...
	ImGui::Begin((E("Undetected Voldo External")), NULL, flags);
	{
